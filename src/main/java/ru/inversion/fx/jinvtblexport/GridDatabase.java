...
import ru.inversion.fx.jinvtblexport.property.Ref;
...
class GridDatabase extends StiDatabase {
    ...
    public GridDatabase( JInvTable grid , SizeExportedDataEnum sed) {
    ...
    private SizeExportedDataEnum sizeExportedData = SizeExportedDataEnum.FULL_TABLE;
    ...
    private void columnsToListID(List<TableColumn> listColumns, Ref<List<String>> listIdColumns){
        for(TableColumn column : listColumns){
            if (!column.isVisible() && sizeExportedData == SizeExportedDataEnum.DISPLAYED_TABLE )
                continue;
    ...
    public void setDataStoreSource( StiDataStoreSource stiDataStoreSource ) throws StiException, DataSetException
    {
        ...
        if(sizeExportedData == SizeExportedDataEnum.FULL_DATA){
            // Считываем класс строки из первой строчки и добавляем к колонкам из JInvTable колонки/поля из RowEntity
            ...
            // Полный список колонок, включая колоники DataSet, плюс к колонкам JInvTable.
