...
public class Report {
    ...
    private SizeExportedData sizeExportedData;
    ...
    public static String getFileOutput(ExportFormat format, String tempDir, String fileName) {
        String exp = "";
    ...
    public static String export(StiReport report, ExportFormat format, String TempDir, String File) throws StiException, IOException {
    ...
    public static String export(JInvTable source, ExportFormat format, String tempDir, String file, SizeExportedData sed) {
//        try {
//            //        try {
////            exportMRT(source, "D:\\");
////        } catch (IOException ex) {
...
jdbc.password={ myUserPassword };
    ...
    public static String executeReport(ReportProperties variable, String tempDir) throws IOException {
        ...
        ReportMode rm = (ReportMode) variable.get(ReportProperties.KEY_REPORT_MODE_PROPERTIES);

        if (rm == null)
            rm = ReportMode.EXPORT;

        try {
            switch (rm) {
                case EXPORT: {
                    ExportFormat ef = ExportFormat.valueOf(variable.get(ReportProperties.KEY_EXPORT_FORMAT_PROPERTIES).toString());
                ...
                case VIEW: {
                    ...
                    result = Report.export(report, ExportFormat.Html, tempDir, null);
    ...
    public StiReport getReport(JInvTable source) {
        return getReport(source, SizeExportedData.FULL_DATA);
    }

    public StiReport getReport(JInvTable source, SizeExportedData sed) {
        sourceTable = source;
        ...
        for (Iterator it = rootColumns.get().iterator(); it.hasNext(); ) {
            ...
            if (colInfo.getСolumn() == null && (colInfo.fieldName.get().isEmpty() && sed != SizeExportedData.FULL_DATA))
                continue;
...
//			            }
    ...
    public void getBandHeaderInfo(boolean ifExportInvisibeColumns) {
        ...
        if (this.sizeExportedData == SizeExportedData.FULL_DATA) {
    ...
    private ReportHeaderBandInfo addBandHeaderInfo(Ref<ArrayList> bandsHeader, double minBandWidth, Ref<ArrayList> columns, Ref<Double> left, TableColumn column, boolean ifExportInvisibeColumns) {
    ...
    private void addColumnInfo4Field(Ref<ArrayList> columns, IEntityProperty field, Ref<Double> left, double minWidth, boolean ifExportInvisibeColumns) {
    ...
    private ReportHeaderBandInfo addBandHeaderInfoDS(Ref<ArrayList> bandsHeader, double minBandWidth, Ref<ArrayList> columns, Ref<Double> left, IEntityProperty field, boolean ifExportInvisibeColumns) {
        //Получение информации о шапке колонки
    ...
    //Получение информации о коллекции колонок
    private void addColumnInfo(Ref<ArrayList> columns, TableColumn tabColumn, Ref<Double> left, double minWidth, boolean ifExportInvisibeColumns) {
