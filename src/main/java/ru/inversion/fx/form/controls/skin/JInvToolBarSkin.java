...
/**
 * Во всем повторяет класс ToolBarSkin, исправляем только TraversalEngine
 * Нельзя наследоваться от ToolBarSkin, так как изменяем конструктор
 */
public class JInvToolBarSkin extends BehaviorSkinBase<ToolBar, ToolBarBehavior> {
    ...
    public JInvToolBarSkin(ToolBar toolbar) {
        ...
        engine = new ParentTraversalEngine(getSkinnable(), new Algorithm() {
            ...
            public Node selectFirst(TraversalContext context)
            {
                ...
                // JAVAKERNEL-853 игнорировать focus на toolbar'е фильтра
                if( overflow && !(toolbar instanceof JInvFilterToolBar) ) {
                    return overflowMenu;
                }
        ...
        toolbar.getItems().addListener((ListChangeListener<Node>) c -> {
            while (c.next()) {
                for (Node n: c.getRemoved()) {
                    ...
                    n.visibleProperty().removeListener( this::requestUpdateListener );
                }
                box.getChildren().addAll(c.getAddedSubList());
                c.getAddedSubList().forEach( n -> n.visibleProperty().addListener( this::requestUpdateListener ) );
            }
            requestUpdate();
        });
    }
...
//        super.layoutChildren();
    ...
    private void addNodesToToolBar() {
        ...
        for (Node node : getSkinnable().getItems()) {
            if (getSkinnable().getOrientation() == Orientation.VERTICAL) {
                ...
                double prefWidth = snapSize(node.prefWidth(-1)) + getSpacing();
                double minWidth = snapSize(node.minWidth(-1)) + getSpacing();
                double widthToAdd = prefWidth > length ? minWidth : prefWidth;
                x += widthToAdd;
            }
            if (x > length) {
                hasOverflow = true;
        ...
        for (Node node : getSkinnable().getItems()) {
            ...
            if (x <= length) {
                ...
                if ( !node.isManaged() || !node.isVisible() ){
                    continue;
                }
    ...
    private void requestUpdateListener( Observable changed ) {
        requestUpdate();
    }
        ...
        private static final CssMetaData<ToolBar,Number> SPACING =
                new CssMetaData<ToolBar,Number>("-fx-spacing",
                    ...
                    public boolean isSettable(ToolBar n) {
                        final JInvToolBarSkin skin = (JInvToolBarSkin) n.getSkin();
