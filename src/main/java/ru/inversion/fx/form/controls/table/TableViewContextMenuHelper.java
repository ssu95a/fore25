...
public class TableViewContextMenuHelper {
    ...
    */
        ...
        for (Object obj : tableColumns) {
            ...
            cmi.setOnAction(e -> {      
                ...
                if ( tableView instanceof JInvTable ){
                    ...
                    //При скрытии колонок теряется тултип, приходится снова его привязывать при показе колонок
                    Platform.runLater(() -> {
                        rebindingToolTipColumn();
                    });
                }
                e.consume();
            });
    ...
    /** Перепривязка тултипов к колонкам */
    private void rebindingToolTipColumn()
    {
        tableView.lookupAll(".column-header").forEach( n -> {
            final TableColumnHeader header = (TableColumnHeader) n;
            TableColumn t = (TableColumn) header.getTableColumn();

            if( t != null && t instanceof JInvTableColumn ) {

                JInvTableColumn column = (JInvTableColumn) t;

                String tooltipText = null;
                if( column.getToolTipText() != null && !column.getToolTipText().isEmpty() )
                {
                    tooltipText = column.getToolTipText();
                }
                else
                {
                    String toolTip = Controls.getTooltipFromBundleByFieldName(column.getFieldName(), g_bundle).orElse(null);
                    if( toolTip != null ) {
                        tooltipText = toolTip;
                    }
                }
